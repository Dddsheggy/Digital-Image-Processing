% for report.pdf only
% use No.1 bottle as an example (left to right)


pic = imread('0small.jpg');
pic = im2double(pic);
mask = imread('mask1.jpg');
mask = rgb2gray(mask);
mask = im2double(mask);
[m,n,c] = size(pic);
mask = imresize(mask, [floor(m), floor(n)],'bicubic');
picclear = pic .* mask;

% to find this function, check getWeightMatrix.m
sumvalue = sum(sum(picclear));
summ = 0;
sumn = 0;

for i=1:m
    for j=1:n
        summ = summ + picclear(i,j)*i;
        sumn = sumn + picclear(i,j)*j;
    end
end
...
%  end of getWeightMatrix.m

% to find this function, check blurImg.m
onematrix = ones(m,n);
k1 = 0.06;
k2 = 0.2;
k3 = 0.3;
...
% to find this function, check getGausskernel.m
Gausskernel = getGausskernel(3,10);
% end of getGausskernel.m
...
% to find this function, check focusNum.m
trans = 1 - maskblur;
picblur = picblur .* trans;
picclear = pic .* maskblur;  
output = picclear + picblur;
output = imresize(output, [floor(m*6), floor(n*6)],'bicubic');
